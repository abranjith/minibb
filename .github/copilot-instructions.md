...
* Admins can create multiple boards (admin permissions are hardcoded in an env var). Boards can have topics and topics can have posts.
* Users can start multiple topics in those boards. For e.g. /vacation, /travel, /food, etc. and within /vacation, they can have topics like "Hiking in the Alps", "Beach Resorts", etc.
* Each topic can have an initial post and multiple replies. For now support we only support text replies and also just one level.
* There can be multiple posts per page, and we use pagination that is configurable by user
* And we allow basic Markdown for formatting posts.
...
* Also follow clean coding principles interms of code structure, separation of concerns, and modularity
* Do not attempt to restart the backend or frontend service, I will take care of restarting it when needed
...
* All api endpoints are in `/api/`. Example routes: `/api/boards`,`/board/:slug`, `/board/:slug/topic/:id`, `/api/board/<id>`, `/api/board/<id>/topics`, `/api//topic/<id>/posts`, etc.
